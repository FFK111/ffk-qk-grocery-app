...
export default function App(): React.ReactNode {
    ...
    const aggregatedItems = useMemo(() => {
        const grouped: { [key: string]: { totalQuantity: number; unit: string; purchased: boolean; category: string } } = {};

        items.forEach(item => {
            if (!grouped[item.name]) {
                grouped[item.name] = { totalQuantity: 0, unit: item.unit, purchased: true, category: item.category };
            }
    ...
    return (
        <div className="min-h-screen bg-gradient-to-br from-slate-100 to-blue-100 flex flex-col">
            ...
            >
                ...
                {saveStatus === 'saved' && <> <CheckIcon className="w-5 h-5" /> <span>Saved!</span> </>}
            </button>
            <button
